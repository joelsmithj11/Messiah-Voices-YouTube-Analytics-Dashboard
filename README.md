# Messiah-Voices-YouTube-Analytics-Dashboard
This project is a Power BI dashboard developed to analyze the performance of the Messiah Voices YouTube channel. The dashboard provides insights into video views, watch time, subscriber growth, impressions, click-through rate (CTR), and content performance trends. It helps identify top-performing videos and understand audience engagement over time.

## Problem Statement

YouTube creators generate large amounts of performance data, making it difficult to manually track content performance, audience engagement, and channel growth.

This project provides an interactive Power BI dashboard that transforms raw YouTube Analytics data into meaningful insights, helping identify top-performing videos, monitor subscriber growth, analyze watch time trends, and evaluate audience engagement metrics.

## Business Objective

- Analyze channel performance using key metrics.
- Identify top-performing content.
- Monitor audience engagement trends.
- Track subscriber growth over time.
- Support content strategy through data-driven insights.

## Dashboard Development Screenshots
<img width="1920" height="1080" alt="Screenshot 1" src="https://github.com/user-attachments/assets/563be7c4-260f-46c1-aaa9-6697f9885eef" />
<img width="1920" height="1080" alt="Screenshot 2" src="https://github.com/user-attachments/assets/b6651df6-6525-4fa2-893f-232f98d18002" />
<img width="1920" height="1080" alt="Screenshot 3" src="https://github.com/user-attachments/assets/c942af5c-a3ab-4856-9dc4-9fbe40207070" />

## Features
- Total Views Analysis
- Watch Time Analysis
- Subscriber Growth Tracking
- Impressions and CTR Monitoring
- Top Performing Videos
- Channel Growth Trends
- Interactive Filters by Video Title and Upload Year
- Dynamic KPI Cards

## Tools and Technologies
- Microsoft Power BI
- Power Query
- DAX (Data Analysis Expressions)
- Microsoft Excel / CSV Dataset
- Data Visualization Techniques

## Dataset Information
The dataset was generated from YouTube Analytics data of the Messiah Voices YouTube channel and contains:

- Video Title
- Upload Date
- Views
- Watch Time (Hours)
- Subscribers Gained
- Impressions
- Click Through Rate (CTR)

## Dashboard Insights
1. Identifies top-performing videos based on views and watch time.
2. Tracks channel growth across different years.
3. Monitors audience engagement using impressions and CTR.
4. Provides an overview of subscriber growth and content reach.

## Requirements

To open and run this project:

- Software Requirements
- Microsoft Power BI Desktop (Latest Version)
- Windows 10/11
- Dataset Requirements
- CSV file containing YouTube Analytics data

## How to Use
- Download the repository.
- Open the .pbix file using Power BI Desktop.
- Refresh the dataset if required.
- Interact with filters and visualizations to explore insights.

## Dashboard Preview
<img width="1865" height="1070" alt="Screenshot - Dashboard" src="https://github.com/user-attachments/assets/ec81fa01-d18c-4dfb-8569-cd6b79e7f0d0" />

Author:
Joel Smith J
MCA
